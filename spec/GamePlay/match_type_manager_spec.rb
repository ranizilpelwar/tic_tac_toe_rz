...
RSpec.describe "a match type manager" do
  ...
  context "method called get_match_type" do
    it "raises an InvalidValueError when the match number is zero" do
      match_type_manager = TicTacToeRZ::MatchTypeManager.new
      expect{ match_type_manager.get_match_type(0) }.to raise_error(TicTacToeRZ::InvalidValueError)
    end

    it "raises an InvalidValueError when the match number is four" do
      match_type_manager = TicTacToeRZ::MatchTypeManager.new
      expect{ match_type_manager.get_match_type(4) }.to raise_error(TicTacToeRZ::InvalidValueError)
    end
