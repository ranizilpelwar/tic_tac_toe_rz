...
RSpec.describe "a game play validator" do
  ...
  shared_examples "evaluate_move" do |description, value, expected|
    it "returns #{expected} when tile is #{description}" do
      expect(TicTacToeRZ::GamePlayValidator.evaluate_move(@game_board, value).is_valid_move).to be expected
    end
  end

  context "method called evaluate_move" do
    it "raises a NilReferenceError when game board is nil" do
      expect{ TicTacToeRZ::GamePlayValidator.evaluate_move(nil, "1") }.to raise_error(TicTacToeRZ::NilReferenceError)
    end
    ...
    it "returns_correct_index_when_tile_is_unoccupied" do
      expect(TicTacToeRZ::GamePlayValidator.evaluate_move(@game_board, "0").index_of_board).to eq(0)
    end
  end
  ...
  context "method called winning_game?" do
    it "raises a NilReferenceError when game board is nil" do
      ...
      expect{ TicTacToeRZ::GamePlayValidator.winning_game?("X", nil, available_spaces) }.to raise_error(TicTacToeRZ::NilReferenceError)
    end

    it "raises a NilReferenceError when available spaces is nil" do
      expect{ TicTacToeRZ::GamePlayValidator.winning_game?("X", @game_board.board, nil) }.to raise_error(TicTacToeRZ::NilReferenceError)
    end

    it "raises an InvalidValueError when player symbol is invalid" do
      available_spaces = [0, 1]
      expect{ TicTacToeRZ::GamePlayValidator.winning_game?("1", @game_board.board, available_spaces) }.to raise_error(TicTacToeRZ::InvalidValueError)
    end
