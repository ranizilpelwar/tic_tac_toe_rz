...
RSpec.describe "a player selection validator" do
  ...
  context "method called valid?" do
    ...
    it "raises a NilReferenceError when input is nil" do
      expect{ TicTacToeRZ::PlayerSelectionValidator.valid?(nil, "X", "Y") }.to raise_error(TicTacToeRZ::NilReferenceError)
    end

    it "raises a NilReferenceError when player_symbol_one is nil" do
      expect{ TicTacToeRZ::PlayerSelectionValidator.valid?("input", nil, "X") }.to raise_error(TicTacToeRZ::NilReferenceError)
    end

    it "raises a NilReferenceError when player_symbol_two is nil" do
      expect{ TicTacToeRZ::PlayerSelectionValidator.valid?("input", "X", nil) }.to raise_error(TicTacToeRZ::NilReferenceError)
    end

    it "raises a InvalidValueError when input is an empty string" do
      expect{ TicTacToeRZ::PlayerSelectionValidator.valid?("", "X", "Y") }.to raise_error(TicTacToeRZ::InvalidValueError)
    end

    it "raises a InvalidValueError when player_symbol_one is an empty string" do
      expect{ TicTacToeRZ::PlayerSelectionValidator.valid?("input", "", "X") }.to raise_error(TicTacToeRZ::InvalidValueError)
    end
