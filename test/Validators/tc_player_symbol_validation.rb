...
class TestPlayerSymbolValidation < Test::Unit::TestCase
  def test_an_empty_string_is_not_a_valid_player_symbol
    input = ""
    assert_raise TicTacToeRZ::InvalidValueError do TicTacToeRZ::PlayerSymbolValidator.valid?(input) end
  end

  def test_a_single_digit_value_is_not_a_valid_player_symbol
    input = "1"
    return_value = TicTacToeRZ::PlayerSymbolValidator.valid?(input)
