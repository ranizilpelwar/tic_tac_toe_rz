...
class GamePlayValidation < Test::Unit::TestCase
  ...
  def test_evaluate_move_raises_a_nil_reference_error_when_game_board_is_nil
    assert_raises(TicTacToeRZ::NilReferenceError) do TicTacToeRZ::GamePlayValidator.evaluate_move(nil, "1") end
  end

  def test_evaluate_move_returns_two_elements
    result = TicTacToeRZ::GamePlayValidator.evaluate_move(@game_board, "1")
    expected_length = 2
  ...
  def test_evaluate_move_returns_false_when_tile_is_occupied
    result = TicTacToeRZ::GamePlayValidator.evaluate_move(@game_board, "4")
  ...
  def test_evaluate_move_returns_false_when_tile_is_an_alpha_character
    result = TicTacToeRZ::GamePlayValidator.evaluate_move(@game_board, "X")
  ...
  def test_evaluate_move_returns_false_when_tile_value_is_out_of_bounds
    result = TicTacToeRZ::GamePlayValidator.evaluate_move(@game_board, "10")
  ...
  def test_evaluate_move_returns_true_when_tile_is_unoccupied
    result = TicTacToeRZ::GamePlayValidator.evaluate_move(@game_board, "0")
  ...
  def test_winning_game_raises_a_nil_reference_error_when_game_board_is_nil
    ...
    assert_raises(TicTacToeRZ::NilReferenceError) do TicTacToeRZ::GamePlayValidator.winning_game?("X", nil, available_spaces) end
  end

  def test_winning_game_raises_a_nil_reference_error_when_available_spaces_is_nil
    assert_raises(TicTacToeRZ::NilReferenceError) do TicTacToeRZ::GamePlayValidator.winning_game?("X", @game_board.board, nil) end
  end

  def test_winning_game_raises_an_invalid_value_error_when_player_symbol_is_invalid
    available_spaces = [0, 1]
    assert_raises(TicTacToeRZ::InvalidValueError) do TicTacToeRZ::GamePlayValidator.winning_game?("1", @game_board.board, available_spaces) end
  end

  def test_winning_game_returns_two_elements
    available_spaces = [0, 1, 2, 3, 5]
    result = TicTacToeRZ::GamePlayValidator.winning_game?("X", @game_board.board, available_spaces)
    expected_length = 2
    assert_equal(expected_length, result.length, "Expecting to receive two elements in the result set.")
  end
